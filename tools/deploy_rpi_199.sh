...
REMOTE_TMP="${REMOTE_TMP:-/tmp/cardputerzero-xiaozhi_0.2.6-m5stack1_arm64.deb}"
...
scp "$REMOTE:$REMOTE_DIR/build/cardputerzero-xiaozhi_0.2.6-m5stack1_arm64.deb" "$ROOT_DIR/build/"
ssh "$REMOTE" "cp '$REMOTE_DIR/build/cardputerzero-xiaozhi_0.2.6-m5stack1_arm64.deb' '$REMOTE_TMP' && echo raspberry | sudo -S dpkg -i '$REMOTE_TMP' && echo raspberry | sudo -S systemctl restart APPLaunch.service"
