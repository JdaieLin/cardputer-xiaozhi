...
ssh "$REMOTE" "echo pi | sudo -S sh -c 'if [ -f /etc/apt/apt.conf ]; then cp /etc/apt/apt.conf /etc/apt/apt.conf.xiaozhi.bak; cat /dev/null >/etc/apt/apt.conf; fi; apt-get update; apt-get install -y libsdl2-dev python3-pip; if [ -f /etc/apt/apt.conf.xiaozhi.bak ]; then mv /etc/apt/apt.conf.xiaozhi.bak /etc/apt/apt.conf; fi'"
ssh "$REMOTE" "pip3 install --user --break-system-packages websockets opuslib"
