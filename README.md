...
# Output: build/cardputerzero-xiaozhi_0.2.7-m5stack1_arm64.deb
```
...
with the package version derived from the tag name, so `v0.2.7` builds
`cardputerzero-xiaozhi_0.2.7-..._arm64.deb`.
...
```bash
./build.sh --device --package
python3 /path/to/prepublish_check.py --deb build/cardputerzero-xiaozhi_0.2.7-m5stack1_arm64.deb --app-dir .
czdev login
czdev publish --deb build/cardputerzero-xiaozhi_0.2.7-m5stack1_arm64.deb
```
...
```bash
scp build/cardputerzero-xiaozhi_0.2.7-m5stack1_arm64.deb pi@192.168.100.199:/tmp/
ssh pi@192.168.100.199 "sudo dpkg -i /tmp/cardputerzero-xiaozhi_0.2.7-m5stack1_arm64.deb"
